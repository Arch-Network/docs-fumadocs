...
pub enum DKGMessage {
    StartDKG {
        message: String,
        participants: HashSet<NetworkPeerId>,
    },
...
pub enum DKGStatus {
    ...
    Finished(String, PublicKeyPackage),
    NetworkCompleted(String, PublicKeyPackage),
}
```
...
sequenceDiagram
    ...
    V1->>V1: Save key_package.json
    V2->>V2: Save key_package.json
    V3->>V3: Save key_package.json
```

Each validator:
- Combines all Round2 packages
- Computes the final `PublicKeyPackage`
- Saves the `key_package.json` file
- Transitions to `Ready` state

### The `key_package.json` File

#### Location and Structure
The key package state is automatically created in:
```text
{data_dir}/{network_mode}/key_manager/key_package.json
```

For example: `./.arch_data/devnet/key_manager/key_package.json`

#### File Format
The file stores the block number, the public key package, and an optional encrypted secret share.

#### What Each Field Represents
- **`pubkey_package`**: The public key package for the validator set
- **`secret_share`**: Optional encrypted FROST secret share for the validator
- **`block_number`**: Block number associated with the saved package
...
At the program boundary, `UtxoMeta` is the compact `txid || vout` identifier used to reference a Bitcoin output. Confirmation and spendability checks are performed by runtime, transaction-pool, Titan, and Bitcoin integration components.
