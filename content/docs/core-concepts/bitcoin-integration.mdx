...
- **UtxoMeta**: Compact Arch program identifier for a Bitcoin UTXO. It stores 36 bytes: a 32-byte transaction ID followed by a 4-byte little-endian output index.
...
Richer metadata such as amount, script, and confirmation status is fetched and validated by the runtime, Titan, and Bitcoin integration layers rather than being stored in `UtxoMeta` itself.
