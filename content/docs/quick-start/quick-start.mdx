...
description: "Get your first program running on Arch Network in under 15 minutes"
---
...
- **Git** (v2.0 or later)
- **Rust** (v1.94.0) - [Install Rust](https://rustup.rs/)
- **Solana/Agave CLI** (v3.1.9 or later) - provides `cargo-build-sbf`
...
<Warning>
**Important**: Arch Network currently uses Rust 1.94.0 for host builds. `cargo-build-sbf` from the Solana/Agave CLI manages its own bundled SBF toolchain.
</Warning>
...
<Tip>
If you encounter any issues during installation, join our [Discord](https://discord.gg/archnetwork) for support.
</Tip>
...
The published local validator image is a Linux `amd64` image. On Apple Silicon Macs, run it through Docker's `linux/amd64` emulation; `arch-cli orchestrate start` uses the Docker Compose configuration that pins this platform.
...
```bash
# Use local source code for development, or when the published image is stale/incompatible
...
#### Option B: Testnet (Remote Bitcoin + Public Arch RPC)

For testnet development against the public Arch Network:

```bash
# 1. Create a configuration profile using the public testnet RPC
...
For local development (Option A/C), use `http://localhost:9002`.
:::
...
<Cards>
  <Card title="Learn More" href="/docs/development/writing-your-first-program">
    Check out our Program Development Guide
  </Card>
  <Card title="Token Development" href="/docs/development/how-to-create-a-fungible-token">
  ...
  <Card title="Community" href="https://discord.gg/archnetwork" external>
...
<Cards>
  <Card title="Discord" href="https://discord.gg/archnetwork" external>
    Get support from the community
  </Card>
  <Card title="Community Discord" href="https://discord.gg/archnetwork" external>
    Report bugs and request help
  </Card>
  <Card title="Documentation" href="https://docs.arch.network" external>
