...
cargo run -p bootnode -- \
    ...
    --p2p-bind-port 19001 \
    --leader-public-key "<LEADER_PUBLIC_KEY>" \
...
# - Titan HTTP API: http://127.0.0.1:8080
# - Titan TCP/subscription endpoint: 127.0.0.1:3030
# - Validator RPC: http://127.0.0.1:9002
```

### Public RPC Endpoints

| Network | Arch RPC URL |
|---------|-------------|
...
```bash
# Configure for testnet (using the public Arch RPC endpoint)
...
```bash
# Configure for mainnet (using the public Arch RPC endpoint)
arch-cli config create-profile mainnet \
    --bitcoin-node-endpoint <YOUR_BITCOIN_RPC_ENDPOINT> \
    --bitcoin-node-username <USERNAME> \
    --bitcoin-node-password <PASSWORD> \
    --bitcoin-network mainnet \
    --arch-node-url https://rpc.mainnet.arch.network \
    --titan-url <YOUR_TITAN_ENDPOINT>
```
