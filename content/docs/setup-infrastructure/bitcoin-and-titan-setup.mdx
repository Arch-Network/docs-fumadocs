...
If the published local validator image is stale or fails at startup, run the local stack from a source checkout:

```bash
arch-cli orchestrate start --local /path/to/arch-network
```
