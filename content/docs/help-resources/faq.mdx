...
- **Image platform**: The published local validator image is Linux `amd64`; on Apple Silicon Macs, make sure Docker is running it as `linux/amd64`
- **Stale image**: If the published image fails or lags behind the source branch you need, run `arch-cli orchestrate start --local /path/to/arch-network`
...
Debugging steps:
1. **Check Service Status**: Use `arch-cli orchestrate validator-status` to check the local validator container
