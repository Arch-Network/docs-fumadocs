...
If you haven't already read [How to write an Arch program](/docs/development/writing-your-first-program), we recommend starting there to get a basic understanding of the program anatomy before going further.
...
# Deploy the program from the build output directory
...
```bash
# Create a new keypair for the oracle state account.
# Account allocation/ownership is submitted as a transaction by your client.
arch-cli account create --keypair-path ~/oracle_state.key
```
...
```bash
# Update oracle data with new information by submitting a RuntimeTransaction
# through the Rust/TypeScript SDK or the send_transaction RPC method.
```
...
```bash
# Test oracle update by sending an instruction transaction from your client.

# Verify the update
arch-cli show <ORACLE_STATE_PUBKEY>
```
...
<Cards>
  ...
  <Card title="Program Development Guide" href="/docs/development/writing-your-first-program">
    Learn the basics of Arch program development
  </Card>
  <Card title="Account Management" href="/docs/development/writing-your-first-program">
