...
- **Rust 1.94.0** and Cargo installed ([Install Rust](https://rustup.rs/))
- **Arch Network CLI** - Install using the [System Requirements guide](/docs/quick-start/requirements)
...
# Fund the token account owner (owner acts as fee payer for transfers and approvals)
arch-cli account airdrop --keypair-path ~/DEMO_KEYS/token_account_owner.key
```
...
```bash
# Create a token account (ATA) for the owner and capture its address
...
# Do NOT use --auto-create-ata here since $TOKEN_ACCOUNT is already an ATA address;
# --auto-create-ata would treat it as an owner and derive a different ATA from it.
...
# The --owner keypair is used as both the transfer authority and the fee payer
...
```bash
# Generate a delegate keypair and fund it (delegate pays fees when transferring)
openssl rand -out ~/DEMO_KEYS/delegate.key 32
arch-cli account airdrop --keypair-path ~/DEMO_KEYS/delegate.key

# Derive the delegate's base58 public key from the keypair
export DELEGATE_PUBKEY=$(NO_COLOR=1 arch-cli account airdrop \
  --keypair-path ~/DEMO_KEYS/delegate.key --amount 0 2>/dev/null \
  | awk -F': ' '/Public key/{print $2}')
echo "Delegate pubkey: $DELEGATE_PUBKEY"
...
```bash
# Transfer tokens using the delegate as the signer (delegate pays fees)
...
```bash
# Burn 10 tokens from the token account (owner pays fees)
arch-cli token burn \
  $TOKEN_ACCOUNT \
  10000000 \
  --owner ~/DEMO_KEYS/token_account_owner.key
...
# The --authority keypair acts as both the freeze authority and the fee payer
...
```bash
# Derive the owner's public key as the destination for remaining lamports
export OWNER_PUBKEY=$(NO_COLOR=1 arch-cli account airdrop \
  --keypair-path ~/DEMO_KEYS/token_account_owner.key --amount 0 2>/dev/null \
  | awk -F': ' '/Public key/{print $2}')

# Close the token account (reclaims rent to owner)
arch-cli token close-account "$TOKEN_ACCOUNT" "$OWNER_PUBKEY" \
  --owner ~/DEMO_KEYS/token_account_owner.key
```
...
The `arch-network` workspace includes an `apl-token-metadata` crate, but the current `arch-cli token` command set does not expose `set-metadata` or `show-metadata` subcommands. Interact with token metadata through client transactions until CLI support is added.
...
# Fund payer and token account owner
arch-cli account airdrop --keypair-path ~/DEMO_KEYS/payer.key
arch-cli account airdrop --keypair-path ~/DEMO_KEYS/token_account_owner.key
...
- Payer (`--keypair-path`): pays fees for account creation (`create-account`, `create-mint`) and multisig operations
- Mint authority (`--authority`): must equal on-chain Mint Authority; also pays fees when used as the signer
- Owner (`--owner`): signs single-signer operations (transfer, approve, burn, close) **and pays fees** for those transactions. Must be funded via airdrop
...
Use the CLI examples above for end-to-end token operations. For custom integrations, construct and submit token instructions with the SDK or the `send_transaction` RPC method.
